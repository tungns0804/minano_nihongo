 ...
 *
 * Mọi màn hình trong app đều nạp động (`loadComponent` ở app.routes.ts), nên bấm
 * một mục menu là trình duyệt phải tải về một tệp JavaScript. Lần đầu vào một tab
 * thì việc đó mất vài trăm mili giây; những lần sau tệp đã nằm trong bộ nhớ đệm
 * nên xong gần như tức thì.
 *
 * Vẽ thanh ngay từ mili giây đầu tiên sẽ khiến trường hợp thứ hai — tức đa số lần
 * bấm — loé lên một cái rồi tắt, nhiễu hơn là không có gì. Ngưỡng này lọc đúng
...
/**
 * Trạng thái "đang chuyển trang", để thanh tiến trình ở khung ngoài biết khi nào
 * cần hiện.
 *
 * Vì sao cần: router chỉ dựng component MỚI sau khi tệp của nó tải xong, nên trong
 * lúc chờ thì màn hình vẫn đứng nguyên ở trang cũ. Không có phản hồi gì thì người
 * dùng tưởng cú bấm bị trượt và bấm thêm lần nữa.
 *
 * Đặt ở đây chứ không nhét thẳng vào `App` để phần hẹn giờ có chỗ đứng riêng và
 * kiểm thử được, còn `App` chỉ việc đọc một signal.
 */
...
export class NavigationProgress {
  ...
  private start(): void {
    // Điều hướng mới chen vào giữa chừng thì huỷ lệnh tắt đang chờ, để thanh chạy
    // liền một mạch thay vì tắt rồi bật lại.
