...
for (const file of files) {
  ...
  for (const match of content.matchAll(KEY_PATTERN)) {
    ...
    if (allKeys.has(key)) {
      usedKeys.add(key);
    } else if (/^(common|app|theme|kind|home|lesson|grammar|practice|result|import|scope|direction|verbMode|verbForm|group|parse|error|route|favorite|exercise|kanji|radical|n3)\./.test(key)) {
